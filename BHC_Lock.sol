...
contract BHC_Lock is Ownable {
    ...
    uint256 private NextMonth = 2366769;
    ...
    function LockTokens(uint256 amount) public onlyOwner {
        if(contractLocked){ revert("This contract is already dealing with locked BHC tokens, to lock up more tokens deploy a similar smart contract."); }
        IERC20(BHC).transferFrom(msg.sender, address(this), amount);
        lockTime = block.timestamp;
        nextReleaseTime = block.timestamp + NextMonth;
        releaseAmount = (amount * 500) / 10000;
        contractLocked = true;
    }
    ...
    function ReleaseTokens() public onlyOwner {
        if(block.timestamp >= nextReleaseTime){
            nextReleaseTime = block.timestamp + NextMonth;
            uint256 bal = IERC20(BHC).balanceOf(address(this));
            if(releaseAmount <= bal){
                IERC20(BHC).transfer(owner(), releaseAmount);
            }else{
