...
contract BHC_Presale is Ownable, ReentrancyGuard {
    ...
    function EnterPresale(uint256 amount) public {
        require(active, "The Presale has not yet started");
        require((amount >= 10 ether), "Minimum amount is 10 BUSD");
        uint256 bal = IERC20(BHC).balanceOf(address(this));
        if(bal >= amount){
            IERC20(BUSD).transferFrom(msg.sender, address(this), amount);
            uint256 p = amount * 20;
            IERC20(BHC).transfer(msg.sender, p);
        }else{
            revert("You must purchase a smaller amount");
        }
    ...
    function WithdrawRemainingBHC() external onlyOwner {
        uint256 bal = IERC20(BHC).balanceOf(address(this));
